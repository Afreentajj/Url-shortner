    ...
    public ResponseEntity<?> generateShortUrlAndQRCode(@RequestBody UrlEntity url) {
        ...
        if (existingUrl != null) {
            ...
            try {
                ...
                response.put("shortUrl", "http://localhost:8880/" + existingUrl.getHashCode());
            ...
            try {
                byte[] qrCodeImage = urlService.generateQRCodeImage("http://localhost:8880/" + url.getHashCode());
                url.setQrCode(qrCodeImage);
                urlRepository.save(url);
                response.put("shortUrl", "http://localhost:8880/" + url.getHashCode());
                String qrCodeBase64 = Base64.getEncoder().encodeToString(qrCodeImage);
                response.put("qrCode", qrCodeBase64);
                return ResponseEntity.ok().body(response);
